...
namespace WinRT.Framework.Utils.Utils
{
    ...
    {
        public async static Task SerializeAsXmlDataContract<T>(this T objectGraph, string fileName, StorageFolder folder = null, bool overwriteIfNull = true)
        {
        ...
        public static async Task<T> LoadFromXmlDataContractFile<T>(string fileName, StorageFolder folder = null)
        {
            var xmlString = await StorageHelper.ReadFromFile(fileName, folder);
        ...
        public async static Task SerializeAsXml<T>(this T objectGraph, string fileName, StorageFolder folder = null, bool overwriteIfNull = true)
        {
            string xmlString;
        ...
        public static async Task<T> LoadFromXmlFile<T>(string fileName, StorageFolder folder = null)
        {
            ...
            var ms = new MemoryStream(Encoding.UTF8.GetBytes(xmlString));
            var ser = new XmlSerializer(typeof(T));
            var result = (T)ser.Deserialize(ms);
            return result;
        }
    }
}
